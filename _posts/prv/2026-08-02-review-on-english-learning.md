...
채팅 세션에서 나온 분석과 이론, 반론 등은 훨씬 더 길지만 요약하면 다음과 같다.

- 그러니까 준의 결론 — "기술적 학습은 할 만큼 했고, 이제 체화 단계다, 인풋을 최대화하자" — 는 이 분석과 정확히 일치해. 자동화와 depth는 분석적 공부로는 만들어지지 않고 대량의 맥락화된 노출로만 만들어지거든.
- **첫째, 양을 냉정하게 계산해봐.** 하루 15분은 연간 약 90시간이야. 자동화에 필요한 노출이 수천 시간 단위라는 걸 감안하면, 이 속도로는 체감 변화가 오기까지 다시 수년이 걸려. 팟캐스트+드라마+오디오북을 합쳐 하루 60~90분을 기본선으로 잡는 게 현실적인 임계량이라고 봐. (리딩을 염두에 둔 게 아니므로 추후 30~60분으로 수정) 리딩도 마찬가지로, 이전에 얘기했던 vividness gap은 분석이 아니라 노출 총량에만 반응하는 영역이야.
