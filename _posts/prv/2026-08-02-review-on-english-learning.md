...
- **첫째, 양을 냉정하게 계산해봐.** 하루 15분은 연간 약 90시간이야. 자동화에 필요한 노출이 수천 시간 단위라는 걸 감안하면, 이 속도로는 체감 변화가 오기까지 다시 수년이 걸려. 팟캐스트+드라마+오디오북을 합쳐 하루 60~90분을 기본선으로 잡는 게 현실적인 임계량이라고 봐. 리딩도 마찬가지로, 이전에 얘기했던 vividness gap은 분석이 아니라 노출 총량에만 반응하는 영역이야.
- **둘째, extensive 일변도로 가지 말고 decoding에 소량의 intensive를 유지해.** 연음·축약·기능어가 안 들리는 건 인풋량만으로는 개선이 느린, 음운 해독의 병목이야(이전에 진단했던 지점 그대로). 하루 10분 정도 짧은 구간 받아쓰기나 shadowing으로 이 병목을 직접 때리면서, 나머지는 extensive로 채우는 배분이 효율이 좋아.
- 현실적인 배분을 제안하면:
    - **듣기 60~90분/일** — 이 중 10분 정도만 intensive(받아쓰기/shadowing), 나머지는 extensive. 단, 이건 "책상에 앉는 시간"일 필요가 없어. 통근, 산책, 집안일에 얹는 시간이라 확보 난이도가 낮아.
    - **읽기 30~60분/일** — 이쪽은 집중이 필요한 시간이라 비용이 커. 그래서 최소선(30분)을 지키는 걸 우선하고, 늘릴 여력이 생기면 읽기부터 늘리는 걸 권해. 지금 준의 어휘 사이즈(15,000)면 소설·논픽션 대부분이 i+1 범위라, 읽기가 depth 전환 효율이 가장 좋은 구간이거든.
